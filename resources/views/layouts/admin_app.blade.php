                    ...
                    <a href="javascript:void(0);" class="header-link dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="d-sm-block d-none">
                                <p class="fw-semibold mb-0 lh-1">{{ $panelAdmin?->name ?? 'Admin' }}</p>
                                <span class="op-7 fw-normal d-block fs-11">{{ $panelAdmin?->email ?? '' }}</span>
                            </div>
                        </div>
                    </a>
        ...
        <div class="main-sidebar" id="sidebar-scroll">
            <nav class="main-menu-container nav nav-pills flex-column sub-open">
                <ul class="main-menu">
                    ...
                    @foreach($allowedModules ?? [] as $moduleKey => $moduleName)
                        @php
                            $crudBase = config('module_admin.routes')[$moduleKey] ?? null;
                            $moduleHref = $crudBase
                                ? route($crudBase.'.index')
                                : route('admin.modules.show', $moduleKey);
                            $routeName = request()->route()?->getName() ?? '';
                            $moduleActive = $crudBase
                                ? ($routeName !== '' && str_starts_with($routeName, $crudBase.'.'))
                                : (request()->routeIs('admin.modules.show') && request()->route('module') === $moduleKey);
                        @endphp
                        <li class="slide">
                            <a href="{{ $moduleHref }}" class="side-menu__item {{ $moduleActive ? 'active' : '' }}">
